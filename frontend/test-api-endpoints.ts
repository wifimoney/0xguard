...
async function runTests() {
  ...
  const agentStatusResult = await testEndpoint('/api/agent-status', 'GET', undefined);
  ...
  // Test 15: POST /api/audit/start - Invalid JSON
  log('Test 15: POST /api/audit/start (invalid JSON - should fail)', 'blue');
  // This would need special handling for invalid JSON, skipping for now
  log('   ⏭️  Skipped (requires raw body test)', 'yellow');
  results.push({
    endpoint: '/api/audit/start',
    method: 'POST',
    status: 'SKIP',
    message: 'Invalid JSON test requires raw body handling',
  });
